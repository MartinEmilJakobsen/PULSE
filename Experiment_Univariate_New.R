...
dat <- expand_grid(nInst=nInst,rho=rho,Rsq=Rsq, nSim = nSim, n=nObsPerSim) %>%
  arrange(-nInst,-n) %>% 
  mutate(Res = future_pmap(list(nSim=nSim, n=n,nInst=nInst,rho=rho,Rsq=Rsq),.f=Simulate,.progress = TRUE)) %>%
  unnest(cols=c(Res))

message("Parallization code has been executed without error")

DataNameWOtimestamp <- paste0("Experiment_Univariate_New_nSim_",paste0(nSim,collapse="_"),"_nObsPerSim_",paste0(nObsPerSim,collapse="_"),"_")
...
file.copy(from="Experiment_Univariate_New.R", to=paste0("Data/Log/",DataNameWOtimestamp,timestamp,".R"), overwrite = TRUE, copy.mode = TRUE, copy.date = FALSE)
